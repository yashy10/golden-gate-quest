...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an expert San Francisco tour guide and travel curator. You help create personalized quests/itineraries.
...
When chatting, be helpful and confirm any changes you make to the itineraries.`;
    ...
    if (type === "generate") {
      // Initial generation - create two distinct itinerary options
      requestBody = {
...
Make sure the two options are meaningfully different so the user has a real choice.`
      ...
      // Chat mode - process user message and potentially modify itineraries
      requestBody = {
        model: "google/gemini-3-flash-preview",
        messages: [
          { role: "system", content: systemPrompt },
    ...
    if (message?.tool_calls?.[0]) {
      // Tool was called - parse the function arguments
